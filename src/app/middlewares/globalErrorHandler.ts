...
const globalErrorHandler = (err: any, req: Request, res: Response, next: NextFunction) => {
  ...
  if (err instanceof ZodError) {
  ...
  } else if (err instanceof Error.ValidationError || err.name === "ValidationError") {  // handle mongoose error
    const errorKeys = Object.keys(err.errors)
    const errorPaths = errorKeys.length > 1
      ? errorKeys.slice(0, -1).join(', ') + ' & ' + errorKeys[errorKeys.length - 1]
      : errorKeys[0];
    ...
    statusCode = 400
    message = errorPaths?.length ? `${errorPaths} is required!` : "Data validation error!"
